...
def mqtt_client1():
    ...
    def on_message(client, userdata, msg):
        ...
        if 'METER' in msg_type:
            if msg_type[6] == "0":  # This is to calculate the energy consumption.
                ...
                try:
                    ...
                    dateHourLowerLimitCheck = current_time.replace(hour=int(current_time.hour), minute=0, second=0,
                                                                   microsecond=0)
                    dateHourUpperLimitCheck = current_time.replace(hour=int(current_time.hour), minute=59, second=59,
                                                                   microsecond=0)
                    ...
                    topictosend = "/Acclivate/iOmniControl/" + str(Site.objects.all()[0].id) + "/" + \
                                  HomeGatewayId.objects.all()[0].hgw_id + "/in/" + "consumption" + "/state"
                    if meter_reading_entry.exists():
                        ...
                        if incoming_meter_reading >= previous_meter_reading_value:
                            ...
                            new_unit_consumption = float(incoming_meter_reading - previous_meter_reading_value) / 1000
                            print("Unit consumption for this leg is {}.".format(str(new_unit_consumption)))
                            hourly_entry = HourlySiteReading.objects.filter(associated_Site=int(location_id),
                                                                            leg_id=leg_id,
                                                                            reading_from__gte=dateHourLowerLimitCheck,
                                                                            reading_from__lte=dateHourUpperLimitCheck)
                            daily_entry = DailySiteReading.objects.filter(associated_Site=int(location_id),
                                                                          leg_id=leg_id,
                                                                          reading_for=current_time.date())
                            print("Now checking condition for entry or update in hourly reading.")
                            dateHourLowerLimitCheck1 = dateHourLowerLimitCheck - timedelta(hours=1)
                            dateHourUpperLimitCheck1 = dateHourUpperLimitCheck - timedelta(hours=1)
                            previous_hour_entry = HourlySiteReading.objects.filter(associated_Site=int(location_id),
                                                                                   leg_id=leg_id,
                                                                                   reading_from__gte=dateHourLowerLimitCheck1,
                                                                                   reading_from__lte=dateHourUpperLimitCheck1)
                            time_difference_in_readings = (
                                        current_time - meter_reading_entry[0].updated_on).total_seconds()
                            print("time difference : ", time_difference_in_readings)
                            print("leg id is : ", leg_id)
                            print("aisle_ grp object: ", aisle_grp)
                            new_cumulative_consumption = aisle_grp[0].cumulative_consumption + new_unit_consumption
                            print("new cumulative consumption  is : ", new_cumulative_consumption)
                            ...
                            if hourly_entry.exists() and previous_hour_entry.exists():
                                ...
                                aisle_grp.update(cumulative_consumption=new_cumulative_consumption)
                                meter_reading_entry.update(previous_reading_value=incoming_meter_reading,
                                                           updated_on=current_time)
                                print("The hourly reading for leg id {} is updated".format(leg_id))
                                print("Meter reading also updated for {}".format(message_for))
                                payload = "Consumption_time_in_sec :" + str(time_difference_in_readings) + \
                                          ",Unit_consumption :" + str(new_unit_consumption) + \
                                          ",saving :0.0" + \
                                          ",Message_time :" + str(current_time) + \
                                          ",Leg_Meter_Reading :" + str(msg_type[3]) + "_" + str(message) + \
                                          ",Current_Hour_total_consumption :" + str(hourly_entry[0].unit_consumption) + \
                                          ",AisleGrp_id :" + str(leg_id) + \
                                          ",GW_Total_Cumulative :" + str(aisle_grp[0].cumulative_consumption) + \
                                          ",Previous_hour_total_consumption :" + str(
                                ...
                                if hourly_entry.exists():
                                    ...
                                    payload = "Consumption_time_in_sec :" + str(time_difference_in_readings) + \
                                              ...
                                              ",Message_time :" + str(current_time) + \
                                              ",Leg_Meter_Reading :" + str(msg_type[3]) + "_" + str(message) + \
                                              ",Current_Hour_total_consumption :" + str(
                                        hourly_entry[0].unit_consumption) + \
                                              ",AisleGrp_id :" + str(leg_id) + \
                                              ",GW_Total_Cumulative :" + str(aisle_grp[0].cumulative_consumption) + \
                                              ",Previous_hour_total_consumption :" + str(previous_hour_entry) + \
                                              ",Rc :1"

                                else:
                                    print("Creating new hourly entry for {}.".format(leg))
                                    HourlySiteReading.objects.create(associated_Site=site, aisle_group=aisle_grp[0],
                                                                     leg_id=leg_id,
                                    ...
                                    meter_reading_entry.update(previous_reading_value=incoming_meter_reading,
                                                               updated_on=current_time)
                                    aisle_grp.update(cumulative_consumption=new_cumulative_consumption)
                                    print("New hourly entry for leg id {} is created.".format(leg_id))
                                    if previous_hour_entry.exists():
                                        previous_hour_units = str(previous_hour_entry[0].unit_consumption)
                                    else:
                                    ...
                                    payload = "Consumption_time_in_sec :" + str(time_difference_in_readings) + \
                                              ...
                                              ",Message_time :" + str(current_time) + \
                                              ",Leg_Meter_Reading :" + str(msg_type[3]) + "_" + str(message) + \
                                              ",Current_Hour_total_consumption :" + previous_hour_units + \
                                              ",AisleGrp_id :" + str(leg_id) + \
                                              ",GW_Total_Cumulative :" + str(new_unit_consumption) + \
                                              ",Previous_hour_total_consumption :" + previous_hour_units + \
                                              ",Rc :1"

                            print("Now checking condition for entry or update in daily reading.")
                            if daily_entry.exists():  # This is the logic for daily site reading.
                                print("Updating daily consumption data.")
                                new_consumption = daily_entry[0].unit_consumption + new_unit_consumption
                                ...
                                DailySiteReading.objects.create(associated_Site=site, aisle_group=aisle_grp[0],
                                                                leg_id=leg_id, unit_consumption=new_unit_consumption,
                                                                reading_for=current_time.date())
                                print("New daily entry for leg id {} is created.".format(leg_id))
                        else:
                            print("ERROR!!!")
                            print(
                                "New meter reading for energy of leg_id {} is smaller than previous meter reading.".format(
                                    leg_id))
                            payload = "ERROR!!!  " + "New meter reading for energy of aisle group id " + str(
                                leg_id) + " is smaller than previous meter reading."
                        client.publish(topictosend, payload, qos=0, retain=False)
                    else:
                        print("Setting the first reference reading for this leg.")
                        MeterReadings.objects.create(local_meterId=int(msg_type[4]), reading_for=0,
                                                     reading_of=str(message_for),
                                                     previous_reading_value=str(message), updated_on=datetime.now())
                        print("Reference reading set for leg {}.".format(msg_type[3]))

                except Exception as e:
                    print("This is the exception in consumption block : {}".format(e))

            if site.site_type == 1:  # These calculations are required only for WH_Metering.
                if msg_type[6] == "2":  # This is to publish the phase wise voltage.
                    try:
                        print("This is for Instantaneous Voltage for site {}.".format(site))
                        voltage_entry = SiteLoadPower.objects.filter(Associated_Site=site,
                        ...
                        if voltage_entry.exists():
                            ...
                            print("Site load entry for this meter has not been created yet!")
                    ...
                    try:
                        print("This is for Instantaneous Current for site {}.".format(site))
                        current_entry = SiteLoadPower.objects.filter(Associated_Site=site,
                                                                     Meter_Number=int(msg_type[4]))
                        if current_entry.exists():
                            if msg_type[5] == '1':
                                ...
                                print("R-phase current updated as {}.".format(r_current))
                                ...
                                print("Y-phase current updated as {}.".format(y_current))
                                ...
                                print("B-phase current updated as {}.".format(b_current))
                ...
                elif msg_type[6] == "4":  # This is to publish for phase wise power factor
                    try:
                        print("This is for Power Factor for site {}.".format(site))
                        power_factor_entry = SiteLoadPower.objects.filter(Associated_Site=site,
                                                                          Meter_Number=int(msg_type[4]))
                        if power_factor_entry.exists():
                            if msg_type[5] == "1":
                                ...
                                print("R-phase power factor updated as {}.".format(r_power_factor))
                                ...
                                print("Y-phase power factor updated as {}.".format(y_power_factor))
                                ...
                                print("B-phase power factor updated as {}.".format(b_power_factor))
        ...
        if site.site_type == 1:  # These messages are received only in WHTM Gateway.
            if "SOURCE" in msg_type:  # This is to send the supply source information and publish load power.
                try:
                    print("This is for power supply source information.")
                    meter_number = int(msg_type[2])
                    meter_info = MeterSource.objects.filter(Associated_Site=site, meter_number=meter_number)[0]
                    ...
                    power_sources = SiteLoadPower.objects.filter(Associated_Site=site, Meter_Number=meter_number)
                    ...
                    topictosend = "/Acclivate/iOmniControl/" + str(Site.objects.all()[0].id) + "/" + \
                                  HomeGatewayId.objects.all()[0].hgw_id + "/in/" + "load" + "/state"
                    now = datetime.now()
                    ...
                    if meter_dual_source:
                        if message == '0':
                            if source_1_entry.exists():
                                ...
                                source_1_entry.update(Status="ON", Updated_on=datetime.now())
                                if source_2_entry.exists():
                                    source_2_entry.update(Site_Total_Load=0, r_volt=0, y_volt=0, b_volt=0, r_current=0,
                                                          r_power_factor=0, y_power_factor=0, b_power_factor=0,
                                                          y_current=0, b_current=0, Status="OFF",
                                                          Updated_on=datetime.now())
                                print("Status updated.")
                            else:
                                print("Creating the first entry for power source {}.".format(power_source_1))
                                SiteLoadPower.objects.create(Associated_Site=site, Meter_Number=meter_number,
                                                             Supply_Source=power_source_1, Status="ON",
                                                             Updated_on=datetime.now())
                                print("First entry for {} successfully created.".format(power_source_1))

                        elif message == '1':
                            if source_2_entry.exists():
                                print("Updating status of {}.".format(power_source_2))
                                source_2_entry.update(Status="ON", Updated_on=datetime.now())
                                if source_1_entry.exists():
                                    source_1_entry.update(Site_Total_Load=0, r_volt=0, y_volt=0, b_volt=0, r_current=0,
                                                          y_current=0, b_current=0,r_power_factor=0,y_power_factor=0,b_power_factor=0,
                                                          Status="OFF",Updated_on=datetime.now())
                                print("Status updated.")
                            else:
                                print("Creating the first entry for power source {}.".format(power_source_2))
                                SiteLoadPower.objects.create(Associated_Site=site, Meter_Number=meter_number,
                                                             Supply_Source=power_source_2, Status="ON",
                                                             Updated_on=datetime.now())
                                print("First entry for {} successfully created.".format(power_source_2))
                        ...
                        if source_1_entry.exists():
                            ...
                            msg1 = "Total_load :" + str(s1.Site_Total_Load) + \
                                   ", R-phase-voltage :" + str(s1.r_volt) + \
                                   ", Y-phase-voltage :" + str(s1.y_volt) + \
                                   ", B-phase-voltage :" + str(s1.b_volt) + \
                                   ", R-phase-current :" + str(source_1_entry[0].r_current) + \
                                   ", Y-phase-current :" + str(s1.y_current) + \
                                   ", B-phase-current :" + str(s1.b_current) + \
                                   ", Power_Source :" + s1.Supply_Source + \
                                   ", Status :" + s1.Status + \
                                   ", Meter_number :" + str(s1.Meter_Number) + \
                                   ", Message_created_time :" + created + \
                                   ", R-phase-power-factor :" + str(s1.r_power_factor) + \
                                   ", Y-phase-power-factor :" + str(s1.y_power_factor) + \
                                   ", B-phase-power-factor :" + str(s1.b_power_factor)
                        ...
                        if source_2_entry.exists():
                            ...
                            msg2 = "Total_load :" + str(s2.Site_Total_Load) + \
                                   ", R-phase-voltage :" + str(s2.r_volt) + \
                                   ", Y-phase-voltage :" + str(s2.y_volt) + \
                                   ", B-phase-voltage :" + str(s2.b_volt) + \
                                   ", R-phase-current :" + str(s2.r_current) + \
                                   ", Y-phase-current :" + str(s2.y_current) + \
                                   ", B-phase-current :" + str(s2.b_current) + \
                                   ", Power_Source :" + s2.Supply_Source + \
                                   ", Status :" + s2.Status + \
                                   ", Meter_number :" + str(s2.Meter_Number) + \
                                   ", Message_created_time :" + created + \
                                   ", R-phase-power-factor :" + str(s2.r_power_factor) + \
                                   ", Y-phase-power-factor :" + str(s2.y_power_factor) + \
                                   ", B-phase-power-factor :" + str(s2.b_power_factor)
                        ...
                        if source_1_entry.exists():
                            ...
                            if message == '1':
                                ...
                                source_1_entry.update(Status="OFF", Updated_on=datetime.now())
                            else:
                                print("ERROR!!")
                                print("This status for power source is invalid.")
                            print("Status updated.")
                        ...
                        msg1 = "Total_load :" + str(s1.Site_Total_Load) + \
                               ...
                               ", Y-phase-voltage :" + str(s1.y_volt) + \
                               ", B-phase-voltage :" + str(s1.b_volt) + \
                               ", R-phase-current :" + str(s1.r_current) + \
                               ", Y-phase-current :" + str(s1.y_current) + \
                               ", B-phase-current :" + str(s1.b_current) + \
                               ", Power_Source :" + s1.Supply_Source + \
                               ", Status :" + s1.Status + \
            ...
            elif "TIME" in msg_type:  # This is to read the DG run time from dual source meter.
                try:
                    ...
                    if "LOAD" in msg_type:
                        ...
                        last_load_time_cummulative = MeterReadings.objects.filter(reading_for=1, reading_of=message_for)
                        if last_load_time_cummulative.exists():
                            previous_load_time_cumulative = float(last_load_time_cummulative[0].previous_reading_value)
                            if current_load_time_cumulative >= previous_load_time_cumulative:
                                dateHourLowerLimitCheck = current_time.replace(hour=current_time.hour, minute=0,
                                                                               second=0, microsecond=0)
                                dateHourUpperLimitCheck = current_time.replace(hour=current_time.hour, minute=59,
                                                                               second=59, microsecond=0)
                                if msg_type[5] == '1':
                                    for_power_source = MeterSource.objects.filter(meter_number=int(msg_type[4]))[
                                        0].power_source_1
                                elif msg_type[5] == '2':
                                    for_power_source = MeterSource.objects.filter(meter_number=int(msg_type[4]))[
                                        0].power_source_2
                                last_load_time_entry = SupplyLoadTimeShare.objects.filter(power_source=for_power_source,
                                                                                          reading_from__gte=dateHourLowerLimitCheck,
                                                                                          reading_from__lte=dateHourUpperLimitCheck)
                                print("current_load_time_cumulative : {}".format(current_load_time_cumulative))
                                print("previous_load_time_cumulative : {}".format(previous_load_time_cumulative))
                                load_time = current_load_time_cumulative - previous_load_time_cumulative
                                if last_load_time_entry.exists():
                                    print("Updating the runtime for {}.".format(power_source))
                                    new_hourly_load_runtime = last_load_time_entry[0].hourly_run_time + load_time
                                    last_load_time_entry.update(hourly_run_time=new_hourly_load_runtime)
                                    last_load_time_cummulative.update(
                                        previous_reading_value=current_load_time_cumulative, updated_on=current_time)
                                    print("The {} load runtime has been updated.".format(power_source))
                                else:
                                    print("Creating the first entry for {}.".format(power_source))
                                    SupplyLoadTimeShare.objects.create(site=site, power_source=for_power_source,
                                                                       hourly_run_time=load_time,
                                                                       reading_from=dateHourLowerLimitCheck,
                                                                       reading_to=dateHourUpperLimitCheck)
                                    last_load_time_cummulative.update(
                                        previous_reading_value=current_load_time_cumulative, updated_on=current_time)
                                    print("First entry created for {}".format(power_source))

                                time_now = datetime.now()
                                created = time_now.strftime("%Y-%m-%d %H:%M:%S.%f")
                                topictosend = "/Acclivate/iOmniControl/" + str(Site.objects.all()[0].id) + "/" + \
                                              HomeGatewayId.objects.all()[0].hgw_id + "/in/" + "SupplyTime" + "/state"
                                msg = "Supply_Source :" + str(for_power_source) + \
                                      ",Runtime :" + str(load_time) + \
                                      ",Last_runtime_cumulative :" + str(current_load_time_cumulative) + \
                            ...
                            print("Setting the reference reading for this power source.")
                            MeterReadings.objects.create(local_meterId=msg_type[4], reading_for=1,
                                                         reading_of=message_for,
                                                         previous_reading_value=message, updated_on=datetime.now())
                except Exception as e:
                ...
                try:
                    ...
                    load_entry = SiteLoadPower.objects.filter(Associated_Site=s, Meter_Number=int(msg_type[3]),
                                                              Status="ON")
                    if load_entry.exists():
                        # print "Inside if."
                        print("Updating the load for meter number {}.".format(msg_type[3]))
                ...
                try:
                    ...
                    load_entry = LoadData.objects.filter(Associated_Site=s, Meter_Number=int(msg_type[3]), )
                    leg_id = load_entry[0].leg_id
                    print("leg_id : ", leg_id)
                    if load_entry.exists():
                        # print "Inside if."
                        print("Updating the load for meter number {}.".format(msg_type[3]))
                        load_entry.update(site_total_load=load_power, Updated_on=time_now, epochTime=epochTime)
                        print("Total power updated.")
                    else:
                        print("#######load entry not exists########")
                    topictosend = "/Acclivate/iOmniControl/" + str(Site.objects.all()[0].id) + "/" + \
                                  HomeGatewayId.objects.all()[0].hgw_id + "/in/" + "LoadData" + "/state"
                    msg = "LoadValue :" + str(load_power) + \
                          ", leg_id :" + str(leg_id) + \
                          ", Meter_Number :" + str(msg_type[3]) + \
                          ", Datetime :" + created + \
...
def mqtt_client2():
    ...
    def on_message(client, userdata, msg):
        ...
        if "remoteAccess" in msg_type:
            ...
            gw_id = HomeGatewayId.objects.all()[0]
            ...
            topicsend = "/Acclivate/iOmniControl/" + str(gw_id.connected_to.id) + "/" + \
                        gw_id.hgw_id + "/in/" + \
                        "remoteAccess" + "/state"
            if action == 'start':
                print("Starting autossh.")
                action = os.system('pgrep autossh | xargs kill -9')
                command = 'autossh -M ' + monitoring_port + ' -fN -o "PubkeyAuthentication=yes" -o "PasswordAuthentication=no"  -R ' + rssh_port + ':localhost:22 aviconn@asem1.aviconn.in'
                print(command)
                autossh = os.system(command)
                count = 0
                while (True):
                    if count >= autossh_retry_count:
                        Rssh_port_check = os.popen("netstat -plant | grep " + rssh_port).read().split("\n")[0]
                        if len(Rssh_port_check) == 0:
                            Rssh_port_check = "Not in listen mode."
                        Rport_status = "Rport_status : " + Rssh_port_check
                        Monitoring_port_check = os.popen("netstat -plant | grep " + monitoring_port).read().split("\n")[
                            0]
                        if len(Monitoring_port_check) == 0:
                            Monitoring_port_check = "Not in listen mode."
                        Mport_status = "Mport_status : " + Monitoring_port_check
                        msg1 = "Autossh failed to start on gateway." + "\n" + Rport_status + "\n" + Mport_status
                        print(msg1)
        ...
        if "sync" in msg_type:  # This block is for data recovery mechanism.
            if "consumption" in msg_subtype:  # This is to recover consumption data.
                try:
                    ...
                    print("This message has been received to recover the lost data on the server for {} seconds"
                          " for aisle group id {}.".format(missed_time_in_secs, aisle_group_id))
                    ...
                    gw_total_cumulative = AisleGroup.objects.filter(site=site, aisle_grp_id=int(aisle_group_id))[0]. \
                        cumulative_consumption
                    topictosend1 = "/Acclivate/iOmniControl/" + str(Site.objects.all()[0].id) + "/" + \
                                   HomeGatewayId.objects.all()[
                                       0].hgw_id + "/in/" + "recovery/" + "dailyConsumption/" + "/state"
                    ...
                    while dateHourLastEntry.date() <= sync_date:
                        ...
                        daily_consumption_entry = legs.filter(leg_id=aisle_group_id,
                                                              reading_for=dateHourLastEntry.date())
                        recovery_date = datetime.strftime(dateHourLastEntry.date(), "%Y-%m-%d")
                    ...
                    while sync_hour >= dateHourLastEntryHour:
                        ...
                        print("These are their corresponding unit consumptions : {}".format(hourly_unit_consumptions))
                        msg = "Aisle_group_id : " + aisle_group_id + \
                            "; Recovery_Hours : " + recovery_hours + \
                            "; Unit_consumptions : " + hourly_unit_consumptions + \
                            "; GW_total_cumulative : " + str(gw_total_cumulative)
                        client.publish(topictosend1, msg, qos=0, retain=False)
                        print("Hourly consumption data for aisle group id {} synced with cloud server.".format(
                            aisle_group_id))
                        time.sleep(2)
                ...
                try:
                    ...
                    print("Message received for recovery of runtime for {}".format(
                    ...
                    while current_time >= last_synced_hour:
                        print("This is the recovery for hour {}".format(last_synced_hour))
                        recovery_hours += datetime.strftime(last_synced_hour, "%Y-%m-%d %H:%M:%S.%f") + ","
                    ...
                    print("These are the recovery hours : {}".format(recovery_hours))
                    print("These are the corresponding load runtime : {}".format(recovered_load_runtime))
                    topictosend = "/Acclivate/iOmniControl/" + str(Site.objects.all()[0].id) + "/" + \
                                  HomeGatewayId.objects.all()[0].hgw_id + "/in/" + "recovery/" + "loadRuntime"
                    msg = "Power_source : " + powerSource + \
                          "; Recovery_hours : " + recovery_hours + \
                          "; Recovery_load_runtime : " + recovered_load_runtime
                    print(msg)
                    client.publish(topictosend, msg, qos=0, retain=False)
