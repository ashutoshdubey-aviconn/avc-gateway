...
def handle_sync_message(client, msg, message, msg_type, msg_subtype, site, current_time):
    ...
    if msg_subtype == "consumption":
        try:
            ...
            while sync_hour >= dateHourLastEntryHour:
                ...
                client.publish(topictosend1, msg_payload, qos=0, retain=False)
                print("Hourly consumption data for aisle group id {} synced with cloud server.".format(aisle_group_id))
                time.sleep(2)
    ...
    if msg_subtype == "loadTime":
        try:
            ...
            print("Message received for recovery of runtime for {}".format(power_source[0].get_power_source_display()))
